...
#if HAS_SQL_SERVER
            int result = new SqlDbClient(conn).Query.Access(SQL, context.Parameters).ExecuteNonQuery();
...
SET @CategoryId = @@IDENTITY");
...
#if HAS_SQL_SERVER
            result = new SqlDbClient(conn).Query.Access(SQL, context.Parameters).ExecuteNonQuery();
        ...
        public void Test_PARAMETER()
        {
            ...
            var dt1 = new SqlDbClient(conn).Query.Access(SQL, new { Id = 7 }).FillDataTable();
            Debug.Assert(dt1.Rows.Count >= 5);

            var dt2 = new SqlDbClient(conn).Query.Access(SQL, context.Parameters).FillDataTable();
            Debug.Assert(dt2.Rows.Count >= 5);

            //if you want to change value of parameter from 7 to 10
            context["Id"].Value = 10;

            dt2 = new SqlDbClient(conn).Query.Access(SQL, context.Parameters).FillDataTable();
...
END";
        ...
        [TestMethod]
        public void Test_MaxCount()
        {
            string query = new SqlBuilder()
                 .SELECT().COLUMNS
                 (
                     ("ProductID".AsColumn() + 1).MAX()
                 )
                 .FROM(nameof(Products))
                 .LIMIT(1)
                 .ToString();

            string sql = "SELECT MAX([ProductID] + 1) FROM [Products] LIMIT 1";
            Debug.Assert(sql == query);
            ...
            sql = "SELECT TOP 1 MAX([ProductID] + 1) FROM [Products]";
            Debug.Assert(sql == query);
